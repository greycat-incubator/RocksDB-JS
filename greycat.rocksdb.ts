import { utility, struct, plugin, Callback, Graph, Constants } from '@greycat/greycat'
import { Promise } from 'es6-promise'
import { java } from '@greycat/j2ts-jre';

export namespace greycatincub.rocksdb {
    ...
    export class RocksDBStorage implements plugin.Storage {
        ...
        get(keys: struct.Buffer, callback: Callback<struct.Buffer>): void {
            ...
            Promise.all(promises).then(function (dbValues: Buffer[]) {
                const result: struct.Buffer = this.graph.newBuffer();
        ...
        lock(callback: Callback<struct.Buffer>): void {
            ...
            this.db.get(PREFIX_KEY, function (err: any, current: Int8Array) {
                ...
                const newValArr: Int8Array = new Int8Array(new Buffer(newVal + ""))

                this.db.put(PREFIX_KEY, newValArr, function (err: any) {
                    if (err != null) {
                        console.log(err);
                        if (callback != null) {
                            callback(null);
                    ...
                    if (callback != null) {
                        var resBuffer: struct.Buffer = this.graph.newBuffer();
